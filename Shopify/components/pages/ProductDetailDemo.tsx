...
import { Product, Media, Video, getMetafield } from '@/types/shopify';
...
import { getProductMedia, isVideo, isImage } from '@/utils/transformers/product-transformer';
...
export default function ProductDetailDemo({ 
  ...
  const allMedia = useMemo(() => {
    try {
      ...
      console.log('🖼️ [PRODUCT DETAIL] Media extraction:', {
        ...
        extractedMediaCount: media.length,
        mediaUrls: media.map(m => isImage(m) ? m.image.url : 'video'),
      });
      return media;
    } catch (error) {
    ...
    <div className={cn("container mx-auto py-8 px-4", className)}>
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 lg:gap-12">
        ...
        <div className="space-y-6">
          <ProductDetailHeaderModuleAdapter product={product} />
          ...
          <AccordionItem value="debug" className="border border-border/50 rounded-lg">
            ...
            <AccordionContent className="px-4 pb-4">
              <div className="space-y-4">
                ...
                {(() => {
                  // Handle both array and edges/node structures
                  const metafieldsArray = Array.isArray(product.metafields) 
                    ? product.metafields 
                    : product.metafields?.edges?.map(e => e.node) || [];
                  const rawDocMetafield = metafieldsArray.find(
                    (m) => m.namespace === 'external' && m.key === '_raw_document'
                  );
                  
                  if (rawDocMetafield) {
                    try {
                      const rawDoc = JSON.parse(rawDocMetafield.value || '{}');
                      return (
                    ...
                    <dl className="space-y-2 text-sm">
                      ...
                      {(() => {
                        ...
                        <div className="mt-3 pt-3 border-t border-border/30 max-h-64 overflow-y-auto">
                          ...
                          <dd className="text-xs space-y-1">
                            {metafieldsArray.map((metafield, idx) => {
